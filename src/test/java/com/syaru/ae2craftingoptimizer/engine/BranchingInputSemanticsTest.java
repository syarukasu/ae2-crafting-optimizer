...
class BranchingInputSemanticsTest {
    ...
    @Test void preservesNbtFuzzyInventoryOrderAndRejectsInvalidVariants() throws Exception {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            item.getOrCreateTag().putInt("variant", i);
    ...
    private static appeng.crafting.pattern.AEProcessingPattern processing(List<GenericStack> inputs) {
        ...
        var ins = new net.minecraft.nbt.ListTag();
        var outs = new net.minecraft.nbt.ListTag();
        for (var input : inputs) ins.add(GenericStack.writeTag(input));
        outs.add(GenericStack.writeTag(stack(OUT, 1)));
        definition.getOrCreateTag().put("in", ins);
        definition.getOrCreateTag().put("out", outs);
    ...
    static IPatternDetails pattern(String name, List<IPatternDetails.IInput> inputs, Map<AEKey, Long> outputs) {
        ItemStack definition = new ItemStack(Items.PAPER);
        definition.getOrCreateTag().putString("recipe", name);
        ...
        return new IPatternDetails() {
            ...
            public GenericStack[] getOutputs() {
                return outputs.entrySet().stream().map(e -> stack(e.getKey(), e.getValue())).toArray(GenericStack[]::new);
            }
        };
    }
