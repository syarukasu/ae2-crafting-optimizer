...
class ReusableByproductAe2OracleTest {
    ...
    static void bootstrap() throws Exception {
        ...
        var field = Bootstrap.class.getDeclaredField("isBootstrapped");
        field.setAccessible(true);
        field.setBoolean(null, true);
        // Populate once even when another headless fixture already initialized the registries.
    ...
    static ICraftingPlan oracleService(ICraftingService service, AEKey out, long amount,
        ...
        var networkField = CraftingCalculation.class.getDeclaredField("networkInv");
        networkField.setAccessible(true);
        var network = networkField.get(job);
        var inventoryField = network.getClass().getDeclaredField("list");
        inventoryField.setAccessible(true);
        var inventory = (KeyCounter) inventoryField.get(network);
        stock.forEach(inventory::set);
