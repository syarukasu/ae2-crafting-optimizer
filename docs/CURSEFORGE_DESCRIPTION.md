...
- Leaves AE2 terminal, storage-watcher, and inventory packet behavior untouched in 1.2.1; the earlier synchronization rewrite Mixins are compatibility-disabled.
...
ACO 1.2.1 also unregisters terminal snapshots, storage-watcher pacing, aggregate refresh coalescing, rolling terminal range packets, and client view coalescing. Their old config keys remain readable but cannot reactivate those paths.
...
- 1.2.1ではAE2本来の端末・Storage Watcher・在庫packet処理を維持し、以前の同期書き換えMixinは互換性のため無効化しています。
...
GTCEu Modern、Mekanism、Advanced AE、Neo ECO AE Extensionは任意連携です。対象MODが無い場合、そのPseudo Mixinは適用されません。Advanced AE Quantum Computerには表示値を変えない実行上限だけを適用します。Neo ECO 20.3.xの独自CPUは、本体のスケジューラとFastPathを維持したままACOのCPU別適応予算・MEグリッド共有予算へ参加できます。容量、構造、表示値、クラフト結果は変更しません。ACO 1.2.0では実受理数を返すバッチアダプタAPIを追加し、以前の安全でない入力集約経路は恒久的に無効化しています。

ACO 1.2.1では、端末スナップショット、Storage Watcher間引き、aggregate refresh集約、端末range packet、client view集約を登録解除しています。以前のConfigキーは読み込めますが、これらの経路を再有効化しません。
