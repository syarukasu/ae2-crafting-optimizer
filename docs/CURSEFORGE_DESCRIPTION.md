...
High-impact controls such as deterministic missing fast-fail, grid-tick deferral, Import/Export Bus operation caps, failed automatic craft-request cooldowns, availability-based pattern ordering, fuzzy Export Bus caching, and successful-plan reuse are disabled by default.
...
If a problem appears, restart once with `enableOptimizer = false` and reproduce it before reporting upstream. Please report ACO-specific issues to the ACO project instead of AE2, GTCEu, Mekanism, or Advanced AE unless the problem also occurs without ACO.
...
GTCEu ModernとMekanismは任意連携です。対象MODが無い場合、その連携処理は動作しません。Advanced AEおよびAdvanced Quantum Engineeringとの共存を想定していますが、保守的なリリースではAdvanced AE Quantum Computerの内部実行処理へMixinしません。
...
欠品の決定論的早期終了、Grid Tick遅延、Import/Export Bus操作数上限、失敗した自動クラフト要求のクールダウン、在庫量によるパターン順変更、Fuzzy Export Busキャッシュ、成功済み計画の再利用など、影響範囲が広い機能は既定で無効です。
...
問題が発生した場合は、まず`enableOptimizer = false`へ変更して再起動し、同じ条件で再現するか確認してください。ACOを外すと発生しない問題は、AE2、GTCEu、Mekanism、Advanced AE側ではなくACO側へ報告してください。
