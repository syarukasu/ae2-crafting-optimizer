...
## Proven upstream arithmetic defect and first patch
The unchanged real interpreter produced MAX * 2 = -2, and MAX * 3 + 1 =
9223372036854775806 instead of 27670116110564327422. An exact assertion suite
fails on these two cases. Implement a source patch for arithmetic stack opcodes
using BigInteger, and reject overflow at legacy long boundaries. Keep this in
the isolated probe source set. This fixes arithmetic, NOT the complete exact
stock/rollback/result API. It does not authorize production VM activation.
