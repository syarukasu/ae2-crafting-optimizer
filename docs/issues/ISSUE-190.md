...
Keep existing AE2/AQE ownership. Reuse immutable exact-input observations on the
planning worker, retaining live validation for dynamic inputs and revalidation
before adoption. Share the existing receipt accounting cache with external CPUs
and avoid transaction revision changes when the scheduling cursor is unchanged.
Forge receives the existing active-step/revision implementation. Verify both
loaders, callback/rebuild counts, cancellation and persistence; no new general
machine executor, production restart or unmeasured runtime performance claim.
